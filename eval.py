...
def main():
    ...
    parser.add_argument(
        "--results_folder", type=str, default="results", help="Path to results folder."
    )
    parser.add_argument(
        "--metrics_folder", type=str, default="results", help="Path to results folder."
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="FREIHAND-VAL,HO3D-VAL,NEWDAYS-TEST-ALL,NEWDAYS-TEST-VIS,NEWDAYS-TEST-OCC,EPICK-TEST-ALL,EPICK-TEST-VIS,EPICK-TEST-OCC,EGO4D-TEST-ALL,EGO4D-TEST-VIS,EGO4D-TEST-OCC",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Should Efficient-HaMeR (KD) be loaded instead.",
    )
...
def run_eval(model, model_cfg, dataset_cfg, device, args):
    ...
    dataset = create_dataset(
        model_cfg, dataset_cfg, train=False, rescale_factor=rescale_factor
    )
    ...
    try:
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            with torch.no_grad():
                ...
                stop_time = time.time()
            evaluator(
                start_time=start_time, end_time=stop_time, output=out, batch=batch
            )
    ...
    if metrics is not None:
        ...
        results_csv = os.path.join(args.metrics_folder, "eval_regression.csv")
        save_eval_result(
            results_csv,
            metrics_dict,
            args.checkpoint,
    ...
    if preds is not None:
        results_json = os.path.join(
            args.results_folder, "%s.json" % args.dataset.lower()
        )
