...
def compute_inference_time(start_time, end_time) -> float:
...
class Evaluator:
    ...
    def __call__(
        ...
        gt_keypoints_2d = (
            batch["keypoints_2d"][:, None, :, :].repeat(1, num_samples, 1, 1)
            * bbox_expand_factor
        )
        ...
        fps = compute_inference_time(start_time, end_time)
        ...
        if hasattr(self, "vertices"):
            self.vertices[self.counter : self.counter + batch_size] = (
        ...
        if hasattr(self, "keypoints_3d"):
            ...
            self.keypoints_3d[self.counter : self.counter + batch_size] = (
                pred_keypoints_3d.squeeze().cpu().numpy()
            )
        if hasattr(self, "fps"):
            self.fps[self.counter : self.counter + batch_size] = fps
...
class EvaluatorPCK:
    ...
    def __call__(self, output: Dict, batch: Dict, opt_output: Optional[Dict] = None):
        ...
        pred_keypoints_2d = (
            pred_keypoints_2d * box_size[:, None, None] + box_center[:, None]
        )
        pred_keypoints_2d = pred_keypoints_2d[:, None, :, :]
        gt_keypoints_2d = batch["orig_keypoints_2d"][:, None, :, :].repeat(
            1, num_samples, 1, 1
        )
