...
from .losses import (
    ...
    Keypoint2DLoss,
    ParameterLoss,
    KeypointDistilationLoss,
    FeatureMapDistillationLoss,
)
from . import MANO
...
class EfficientHAMER(pl.LightningModule):

    def __init__(
        self, cfg: CfgNode, teacher_model: HAMER = None, init_renderer: bool = True
    ):
        """
        Setup HAMER model
        Args:
        ...
        self.save_hyperparameters(
            logger=False, ignore=["init_renderer", "teacher_model"]
        )
        ...
        self.keypoint_distilation_loss = KeypointDistilationLoss(loss_type="l1")
        ...
        # 1x1 convolution for adjusting the channel output for the MANO HEAD (for now only for mobinenetv3)
        # if self.cfg.MODEL.BACKBONE.TYPE == "mobilenet_v3":
        #     self.channel_adjustment = torch.nn.Conv2d(
        #         in_channels=960, out_channels=1280, kernel_size=1
        # )
        ...
        # Add a channel projection layer
        if (
            self.cfg.GENERAL.DISTILLATION == "features"
            or self.cfg.GENERAL.DISTILLATION == "full"
        ):
            student_spatial = (
                self.cfg.MODEL.BACKBONE.SPATIAL_HEIGHT,
                self.cfg.MODEL.BACKBONE.SPATIAL_WIDTH,
            )
            teacher_spatial = (16, 12)

            if (
                self.cfg.MODEL.BACKBONE.FEATURE_DIM != 1280
            ):  # Only do this if d is different for teacher and student
                self.channel_projection = torch.nn.Conv2d(
                    1280, self.cfg.MODEL.BACKBONE.FEATURE_DIM, kernel_size=1
                )

            self.spatial_scale = (
                student_spatial[0] / teacher_spatial[0],
                student_spatial[1] / teacher_spatial[1],
            )
            self.feature_map_loss = FeatureMapDistillationLoss()
    ...
    def forward_step(self, batch: Dict, train: bool = False) -> Dict:
        ...
        if self.cfg.MODEL.BACKBONE.TYPE == "vit":  # for vit backbone
        ...
        output["feature_maps"] = conditioning_feats
    ...
    def FeatureDistillation(self, batch: Dict, output: Dict):
        teacher_backbone = self.teacher.backbone
        with torch.no_grad():
            teacher_features = teacher_backbone(batch["img"][:, :, :, 32:-32])

        if (
            self.cfg.MODEL.BACKBONE.FEATURE_DIM != 1280
        ):  # Only do this if d is different for teacher and student
            aligned_features = self.channel_projection(teacher_features)
        else:
            aligned_features = teacher_features

        aligned_features = torch.nn.functional.interpolate(
            aligned_features,
        ...
        loss_feature_maps = self.feature_map_loss(
            aligned_features, output["feature_maps"]
        )

        return loss_feature_maps

    def OutputDistillation(self, batch: Dict, output: Dict):
        ...
        teacher_keypoints_3d = teacher_logits["pred_keypoints_3d"]
        teacher_keypoints_2d = teacher_logits["pred_keypoints_2d"]
        teacher_mano_params = teacher_logits["pred_mano_params"]

        # Compute KD keypoint loss
        teacher_loss_keypoints_2d, teacher_loss_keypoints_3d = (
            self.keypoint_distilation_loss(
                (pred_keypoints_2d.clone(), pred_keypoints_3d.clone()),
                (teacher_keypoints_2d.clone(), teacher_keypoints_3d.clone()),
            )
        )

        # Compute KD loss on MANO parameters
        loss_teacher_mano_params = {}
        for k, pred in pred_mano_params.items():
            teacher_has_params = (
                teacher_mano_params[k]
                .any(dim=tuple(range(1, teacher_mano_params[k].dim())))
                .float()
            )
            teacher_t = teacher_mano_params[k].view(batch_size, -1)
            loss_teacher_mano_params[k] = self.mano_parameter_loss(
                pred.reshape(batch_size, -1),
                teacher_t.reshape(batch_size, -1),
                teacher_has_params,
            )

        return (
            teacher_loss_keypoints_2d,
            teacher_loss_keypoints_3d,
            loss_teacher_mano_params,
        )
    ...
    def compute_loss(
        ...
        if (
            ...
            loss_teacher_features = (
                self.FeatureDistillation(batch, output)
                * self.cfg.LOSS_WEIGHTS.FEATURE_DISTILLATION_SCALER
            )

            losses["loss_teacher_features"] = loss_teacher_features.detach()

        if (
            self.cfg.GENERAL.DISTILLATION == "outputs"
            or self.cfg.GENERAL.DISTILLATION == "full"
        ):
            (
                teacher_loss_keypoints_2d,
                teacher_loss_keypoints_3d,
                loss_teacher_mano_params,
            ) = self.OutputDistillation(batch, output)

            losses["loss_teacher_keypoint_2d"] = teacher_loss_keypoints_2d.detach()
            losses["loss_teacher_keypoint_3d"] = teacher_loss_keypoints_3d.detach()

            for k, v in loss_teacher_mano_params.items():
                losses["teacher_loss_" + k] = v.detach()

        # print(loss_teacher_features, teacher_loss_keypoints_2d, teacher_loss_keypoints_3d, loss_teacher_mano_params, " => KD Losses")

        kd_loss = (
            loss_teacher_features
            + self.cfg.LOSS_WEIGHTS["KEYPOINTS_2D"] * teacher_loss_keypoints_2d
            + self.cfg.LOSS_WEIGHTS["KEYPOINTS_3D"] * teacher_loss_keypoints_3d
            + sum(
                [
                    loss_teacher_mano_params[k] * self.cfg.LOSS_WEIGHTS[k.upper()]
                    for k in loss_teacher_mano_params
                ]
            )
        )

        combined_loss = loss + (
            self.cfg.LOSS_WEIGHTS["KNOWLEDGE_DISTILATION"] * kd_loss
        )

        losses["kd_loss"] = kd_loss.detach()
        losses["combined_loss"] = combined_loss.detach()
